...
- 截取以指定坐标为中心、可自定义地面边长的最新 Esri 卫星影像；输出像素按 Esri 第 19 级瓦片的标称地面采样距离自动计算，并显示 m/px 与每像素地面面积。
...
2. 设置地面范围；面板会按该位置 Esri 第 19 级瓦片自动选择输出像素，并实时显示标称分辨率和每像素地面面积。点击“截取并加入区域库”，系统生成当前区域 PNG 并请求当前 POI。
...
高德 POI 与 Agnes 2.5 演示代理都在服务端读取密钥：
...
每个区域从 Esri World Imagery 的第 19 级瓦片推导标称分辨率，再依据地面边长自动得到正方形输出像素；输出上限为 2048 px，以保证浏览器裁切和后续传输可靠。区域库保存 PNG；发送 Agnes 时会另外生成最长边不超过 1024 px 的 JPEG 传输副本，不改变导出数据或区域库中的原始截取。
