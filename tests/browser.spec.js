...
test('region card loads POI then retrieves the closest historical image for a time point', async ({
  ...
  await page.getByLabel('地面范围（米）').press('Tab');
  await expect(page.getByText(/每像素/)).toBeVisible();
  await page.getByRole('button', { name: '截取并加入区域库' }).click();
  await expect(page.getByTestId('poi-description')).toContainText('和平饭店');
  await expect(page.getByText(/400 m × 400 m · \d+ px/)).toBeVisible();
  ...
  const modelInitial = await modelPanel.boundingBox();
  ...
  const modelAfterLibraryDrag = await modelPanel.boundingBox();
  expect(modelAfterLibraryDrag.y).toBeCloseTo(modelInitial.y, 0);
