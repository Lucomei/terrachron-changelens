...
区域取得历史影像后，可将该历史 PNG、当前 POI 语义和同一范围的 WGS84 空间元数据发送给甲方本地模型。前端不计算传统特征差值，也不内置模型；模型服务可以使用千问 2.5 或其他实现，只要遵守 [模型接口契约](docs/api.md#本地模型变化检测接口) 即可。

复制 `.env.example` 为 `.env.local`，填写本地模型地址后重启 Vite：

```text
VITE_MODEL_API_BASE=http://127.0.0.1:8000/api/v1
```

这是浏览器可见的服务地址，不能填写 API Key。模型服务须允许开发页面 Origin 跨域访问。未配置时“模型变化检测”保持禁用，其他截取、POI、历史影像和导出功能不受影响。
...
src/
  ...
  services/
    ...
    poi.js              同源 POI 服务调用
    model-client.js     本地模型 V1.0 multipart 请求与健康检查
    change-result.js    Markdown 与 JSON 结果的稳定渲染模型
